...
<div class="bg-white shadow-xl rounded-2xl border border-deep-teal-200 overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="mt-2 text-deep-teal-200 text-sm">
                    ...
                    @elseif(auth()->check() && auth()->user()->hasRole('coordinador'))
                        Gestiona las materias asignadas a tu coordinación
                    @else
    ...
    <div class="p-4 sm:p-6 bg-gradient-to-br from-gray-25 to-deep-teal-25">
        ...
        <div class="mb-6 bg-white rounded-2xl border border-deep-teal-100 p-4 sm:p-6 shadow-sm">
            <form action="{{ route('admin.materias.index') }}" method="GET" id="searchForm">
        ...
        @if($materias->count() > 0)
            ...
            <div class="block sm:hidden space-y-4">
                ...
                <div class="bg-white border border-deep-teal-100 rounded-2xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    ...
                    <div class="flex gap-2 mt-3 pt-3 border-t border-deep-teal-100">
                        <a href="{{ route('admin.materias.edit', $materia->sigla) }}" 
                           class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold rounded-xl transition-all duration-200 shadow-md hover:shadow-lg">
                        ...
                        @if(auth()->check() && auth()->user()->hasRole('admin'))
                        <form action="{{ route('admin.materias.destroy', $materia->sigla) }}" method="POST" class="flex-1" 
                              onsubmit="return confirm('¿Está seguro de eliminar la materia {{ $materia->sigla }}? Esta acción no se puede deshacer.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
                                    class="w-full inline-flex items-center justify-center px-3 py-2 bg-red-500 hover:bg-red-600 text-white text-xs font-semibold rounded-xl transition-all duration-200 shadow-md hover:shadow-lg {{ $materia->grupoMaterias->count() > 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
                                    {{ $materia->grupoMaterias->count() > 0 ? 'disabled' : '' }}>
                    ...
                    <tbody class="bg-white divide-y divide-deep-teal-50">
                        ...
                        <tr class="hover:bg-deep-teal-25 transition-all duration-200">
                            ...
                            <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.materias.show', $materia->sigla) }}" 
                                       class="inline-flex items-center px-3 py-2 bg-[#3CA6A6] hover:bg-[#026773] text-white text-xs font-bold rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                                        <i class="fas fa-eye mr-1"></i>
                                        Ver
                                    </a>
                                    
                                    <a href="{{ route('admin.materias.edit', $materia->sigla) }}" 
                                       class="inline-flex items-center px-3 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                                        <i class="fas fa-edit mr-1"></i>
                                        Editar
                                    </a>
                                    
                                    <a href="{{ route('admin.materias.asignar-aulas', $materia->sigla) }}" 
                                       class="inline-flex items-center px-3 py-2 bg-purple-500 hover:bg-purple-600 text-white text-xs font-bold rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                                    ...
                                    <!-- Botón Eliminar - SOLO PARA ADMIN -->
                                    @if(auth()->check() && auth()->user()->hasRole('admin'))
                                    <form action="{{ route('admin.materias.destroy', $materia->sigla) }}" method="POST" 
                                          onsubmit="return confirm('¿Está seguro de eliminar la materia {{ $materia->sigla }}? Esta acción no se puede deshacer.')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                                                class="inline-flex items-center px-3 py-2 bg-red-500 hover:bg-red-600 text-white text-xs font-bold rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 {{ $materia->grupoMaterias->count() > 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
                                                {{ $materia->grupoMaterias->count() > 0 ? 'disabled' : '' }}
                                                title="{{ $materia->grupoMaterias->count() > 0 ? 'No se puede eliminar porque tiene grupos asignados' : 'Eliminar materia' }}">
                                            <i class="fas fa-trash mr-1"></i>
                                            Eliminar
                                        </button>
                                    </form>
